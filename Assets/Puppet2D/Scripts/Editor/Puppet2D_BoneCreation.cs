...
namespace Puppet2D
{
	...
	public class Puppet2D_BoneCreation : Editor
	{
		...
		public static void sortOutBoneHierachy(GameObject changedBone, bool move = false)
		{
			...
			//Puppet2D_Bone Puppet2D_Bone = changedBone.GetComponent<Puppet2D_Bone>();
			/*if (Puppet2D_Bone)
				if (Puppet2D_Bone.sprite)
					if (!Puppet2D_Bone.sprite.name.Contains("Bone"))
						return;*/
			...
			if (changedBonesParent)
			{
				...
				Puppet2D_Bone sprParent = changedBonesParent.GetComponent<Puppet2D_Bone>();
				if (sprParent)
				{
			...
			if (children.Count > 0)
			{
				foreach (Transform child in children)
				{
					Puppet2D_Bone spr = child.GetComponent<Puppet2D_Bone>();
					if (spr)						
					{
						Undo.RecordObject(child, "parents child");
			...
			/*else
			{
				Undo.RecordObject(Puppet2D_Bone, "sprite change");
				Puppet2D_Bone.sprite = Puppet2D_Editor.boneNoJointSprite;
			}*/
			...
			if (changedBonesParent)
			{
				...
				/*Puppet2D_Bone spr = changedBonesParent.GetComponent<Puppet2D_Bone>();
				if (spr)
				{
					if (spr.sprite)
					{
			...
			{
				if (children.Count > 0)
					changedBone.GetComponent<Puppet2D_Bone>().sprite = Puppet2D_Editor.boneSprite;
				else
					changedBone.GetComponent<Puppet2D_Bone>().sprite = Puppet2D_Editor.boneNoJointSprite;
		...
		public static GameObject BoneCreationMode(Vector3 mousePos)
		{
			...
			if (Selection.activeGameObject)
			{
				...
				if (Selection.activeGameObject.GetComponent<Puppet2D_Bone>())
				{
			...
			if (Selection.activeGameObject)
			{
				...
				Puppet2D_Bone Puppet2D_BoneInvisbile = newInvisibleBone.AddComponent<Puppet2D_Bone>();
				newInvisibleBone.transform.position = new Vector3(10000, 10000, 10000);
				//Puppet2D_BoneInvisbile.sortingLayerName = Puppet2D_Editor._boneSortingLayer;
				//Puppet2D_BoneInvisbile.sprite = Puppet2D_Editor.boneHiddenSprite;
			...
			Puppet2D_Bone Puppet2D_Bone = newBone.AddComponent<Puppet2D_Bone>();
			var view = SceneView.lastActiveSceneView;
			if (view != null)
			{
				Puppet2D_Bone.Radius = view.camera.orthographicSize/40f;
			}
			//Puppet2D_Bone.sortingLayerName = Puppet2D_Editor._boneSortingLayer;
		...
		public static void BoneMoveMode(Vector3 mousePos)
		{
			...
			if (selectedGO)
			{
				...
				if (selectedGO.GetComponent<Puppet2D_Bone>())
				{					
					// MAKE SURE SELECTION IS NOT AN IK OR PARENT
